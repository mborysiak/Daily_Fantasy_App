...
class PullData:
    ...
    def __init__(self, week, year, filename, op_params):
        ...
        self.reg_ens_vers = op_params['reg_ens_vers']
...
@st.cache_data
def auto_select(selected, _sim, op_params, stack_team):
    num_selected = selected.my_team.sum()
    while num_selected < 9:
        
        results, team_cnts = run_sim(selected, _sim, op_params, stack_team)
        ...
        if authentication_status:
            ...
            with st.sidebar:
                ...
                if st.button("Refresh Data"):
                    ...
                    st.session_state["dd"] = get_display_data(player_data)
                ...
                if st.button("Auto Select"):
                    st.session_state["dd"] = auto_select(st.session_state["dd"], sim, op_params, stack_team)

                st.header('Pull all teams into CSV for upload')
                st.download_button(
            ...
            with col1:
                ...
                selected = create_interactive_grid(st.session_state["dd"])
